    ...
    <div className="flex-1 flex overflow-hidden bg-surface-50">
      ...
      <div className="flex-1 p-6 flex flex-col gap-6 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-surface-100 rounded-xl border border-surface-400 p-4 shadow-skeu-raised flex flex-col items-center justify-center transform transition-all duration-300 hover:scale-105">
            <Send className="text-blue-500 mb-2" size={24} />
            <span className="text-sm text-surface-600 font-medium">Emails Sent</span>
            <span className="text-3xl font-bold text-surface-900">{sentCount}</span>
          </div>
          
          <div className="bg-surface-100 rounded-xl border border-surface-400 p-4 shadow-skeu-raised flex flex-col items-center justify-center transform transition-all duration-300 hover:scale-105">
            <Mail className="text-amber-500 mb-2" size={24} />
            <span className="text-sm text-surface-600 font-medium">Opened</span>
            <span className="text-3xl font-bold text-surface-900">{openCount}</span>
          </div>

          <div className="bg-surface-100 rounded-xl border border-surface-400 p-4 shadow-skeu-raised flex flex-col items-center justify-center transform transition-all duration-300 hover:scale-105">
            <Inbox className="text-emerald-500 mb-2" size={24} />
            <span className="text-sm text-surface-600 font-medium">Replies</span>
            <span className="text-3xl font-bold text-surface-900">{replyCount}</span>
          </div>
        </div>

        {/* Chart Mock (Animated Bars) */}
        <div className="flex-1 bg-surface-100 border border-surface-400 rounded-xl shadow-skeu-inset p-4 flex items-end gap-2 overflow-hidden relative">
          {[40, 60, 45, 80, 50, 75, 90, 65, 85, 100].map((height, i) => (
            <div 
              key={i} 
              className="flex-1 bg-gradient-to-t from-ice-600 to-ice-400 rounded-t-sm shadow-sm origin-bottom animate-pulse"
              style={{ 
                height: `${height}%`, 
          ...
          <div className="absolute z-10 bottom-1/4 right-1/4 animate-bounce" style={{ animationDuration: '3s' }}>
            <MousePointer2 className="text-surface-900 drop-shadow-md" size={32} />
          </div>
        </div>
      </div>
    </div>
