...
export default function CampaignPage() {
  ...
  if (!data || data.error) return <div className="p-8 text-red-500">Error: {data?.error || 'Failed to load'}</div>
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <span className={`skeu-badge skeu-badge-${data.status.toLowerCase()}`}>{data.status}</span>
        </div>
        
        <div className="flex gap-2">
          {data.status === 'SENDING' && (
            <button onClick={() => handleAction('pause')} className="skeu-btn-ghost flex items-center gap-1">
              <Pause size={16} /> Pause
            </button>
          )}
          {data.status === 'PAUSED' && (
            <button onClick={() => handleAction('resume')} className="skeu-btn-primary flex items-center gap-1">
              <Play size={16} /> Resume
            </button>
          )}
          <button onClick={() => handleAction('duplicate')} className="skeu-btn-ghost flex items-center gap-1">
        ...
        {progress ? (
          <div>
            <div className="flex justify-between text-sm mb-2 text-surface-900 font-medium">
              <span>{progress.sent} Sent / {progress.total} Total</span>
              <span>{Math.round(((progress.sent + progress.failed) / progress.total) * 100) || 0}%</span>
            </div>
            <div className="skeu-progress-bar mb-4">
              <div style={{ width: `${Math.round(((progress.sent + progress.failed) / progress.total) * 100) || 0}%` }}></div>
            </div>
            <div className="flex gap-4 text-sm font-medium">
              <span className="text-green-600">{progress.sent} Sent</span>
              <span className="text-red-600">{progress.failed} Failed</span>
              <span className="text-surface-600">{progress.pending} Pending</span>
            </div>
          </div>
        ) : (
