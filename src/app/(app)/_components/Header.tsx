...
export default function Header() {
 ...
 <header className="h-16 border-b border-border-subtle bg-bg-subtle flex items-center justify-between px-4 md:px-6 sticky top-0 z-50 shrink-0">
  <div className="flex items-center text-sm text-text-muted flex-1 min-w-0 overflow-x-auto whitespace-nowrap mr-2 sm:mr-4 scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
  <Button 
  variant="none"
  onClick={() => router.back()} 
  ...
  <Link href="/dashboard" className="hover:text-text-primary transition-colors shrink-0">
  Home
  </Link>
  ...
  {isLast ? (
  <span className="text-text-primary font-medium truncate max-w-[120px] sm:max-w-[200px]" title={title}>{title.substring(0, 20)}</span>
  ) : (
  <Link href={`/${segments.slice(0, index + 1).join('/')}`} className="hover:text-text-primary transition-colors truncate max-w-[100px] sm:max-w-[150px]" title={title}>
