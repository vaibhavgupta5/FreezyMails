...
export default function TemplateForm({ initialData }: TemplateFormProps) {
 ...
 attachments: (initialData.attachments as TemplateAttachment[] | undefined) || [],
 }
 : {
 name: "",
 subject: "",
 ...
 <Popover open={showAiConfig} onOpenChange={setShowAiConfig}>
 ...
 <PopoverContent
 className="w-80 p-4 space-y-3 bg-bg-subtle border-border-subtle"
 align="end"
 >
 ...
 />
 {errors.name && (
 <p className="text-danger-text text-xs mt-1">{errors.name.message}</p>
 )}
 </div>
 ...
 />
 {errors.body && (
 <p className="text-danger-text text-xs mt-1">{errors.body.message}</p>
 )}
 ...
 >
 <span className="font-mono text-xs truncate max-w-[200px]" title={att.filename}>
 {att.filename}
 </span>
 <Button
 type="button"
