...
export default function AudienceDetailPage() {
 ...
 setContacts([
 ...data.contacts,
 { id: `new-${Date.now()}`, email: "", customFields: {}, isNew: true },
 ]);
 } catch {
 ...
 {[...Array(10)].map((_, i) => (
 <div key={i} className="h-12 border-b border-border-subtle flex items-center px-4 gap-4">
 <Skeleton className="h-4 w-4 rounded" />
 <Skeleton className="h-4 w-48" />
 <Skeleton className="h-4 w-32" />
 <Skeleton className="h-4 w-32" />
 ...
 <TooltipContent side="bottom">Back to Mailing Lists</TooltipContent>
 </Tooltip>
 <div>
 ...
 Manage spreadsheet contacts, custom attributes, and export ready
 lists.
 </p>
 </div>
 </div>
 ...
 <Button variant="outline" size="sm" className="h-8 gap-1.5 text-xs bg-bg-base border-border-subtle relative">
 ...
 <PopoverContent className="w-72 p-0 bg-bg-base border-border-subtle shadow-2xl rounded-xl overflow-hidden" align="start">
 <div className="bg-bg-base/50 px-4 py-3 border-b border-border-subtle flex items-center justify-between">
 <h4 className="font-semibold text-sm text-text-primary">Filter Mailing List</h4>
 {activeFiltersCount > 0 && (
 ...
 <label className="text-xs font-medium text-text-muted">Search Column</label>
 <Select
 value={filterColumn}
 onValueChange={(val) => {
 setFilterColumn(val);
 ...
 <div className="space-y-1">
 <label className="text-xs font-medium text-text-muted">Row Status</label>
 <Select
 value={filterStatus}
 onValueChange={(val) => {
 setFilterStatus(val);
 ...
 <SelectItem value="populated">Complete rows</SelectItem>
 ...
 <div className="space-y-1">
 <label className="text-xs font-medium text-text-muted">Delivery Status</label>
 <Select
 ...
 <TooltipContent>View Campaign History</TooltipContent>
 </Tooltip>
 )}
 </div>
 </TableCell>
 ...
 <div>
 <p className="text-xs text-text-muted font-medium mb-1">Subject:</p>
 <p className="text-xs text-text-primary bg-bg-base p-2 rounded border border-border-subtle">{rec.campaign.template.subject}</p>
 </div>
 <div>
 <p className="text-xs text-text-muted font-medium mb-1">Body Template:</p>
