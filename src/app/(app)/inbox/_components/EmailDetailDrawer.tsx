...
export default function EmailDetailDrawer({
  ...
  useEffect(() => {
    if (reply) {
      setResponseSubject(reply.subject?.startsWith('Re:') ? reply.subject : `Re: ${reply.subject || ''}`);
      setResponseText('');
    }
  }, [reply?.id]);
  ...
  const sentimentBadges = {
    ...
    negative: (
      <span className="skeu-badge bg-red-500/10 text-red-600 border-red-500/20 text-xs py-0.5 px-2">
        🔴 Not Interested
      </span>
    ),
    ooo: (
  ...
  // @ts-ignore - mailEvents populated via API
  const mailEvents = reply.recipient.mailEvents || [];
  const opens = mailEvents.filter((e: any) => e.type === "OPENED").length;
  const clicks = mailEvents.filter((e: any) => e.type === "CLICKED").length;
  ...
  const onSend = async () => {
    ...
    try {
      ...
      if (errMsg.includes("has disconnected") || errMsg.includes("No active email account")) {
        toast.error(errMsg, {
          action: {
      ...
      <div className="absolute top-6 right-6 flex items-center gap-2 z-10">
        {!isSent && (
          <button
            ...
            className={`h-8 w-8 flex items-center justify-center rounded-full transition-colors cursor-pointer border  -sm ${reply.isFlagged ? "bg-amber-500/10 text-amber-500 border-amber-500/20" : "bg-bg-base text-text-muted hover:text-text-primary border-border-subtle"}`}
            title={
        ...
        <div className="mt-8 pt-6 border-t border-border-subtle pb-6">
          {isSent && (
            <div className="mb-4 bg-primary-base/10 rounded-lg p-3 text-primary-base text-xs border border-primary-base/20 font-medium">
              You are sending a direct follow-up to this outbound campaign email. This will create a new thread.
            </div>
          )}
              ...
              <div className="flex  items-center gap-2 relative mb-2">
                ...
                <Popover open={templatesOpen} onOpenChange={setTemplatesOpen}>
                  <PopoverTrigger asChild>
                    <Button variant="ghost" size="sm" className="h-8 text-xs text-text-muted hover:text-text-primary bg-bg-subtle border border-border-subtle">
                      Templates
                      <ChevronDown size={12} />
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent align="end" className="w-[250px] p-1 max-h-[300px] overflow-y-auto">
                    {templates.length === 0 ? (
                      <div className="text-xs text-text-muted px-4 py-2">No templates found.</div>
                    ) : (
                      templates.map((tpl) => (
                        <button
                          key={tpl.id}
                          ...
                          onClick={() => {
                            ...
                            if (reply?.recipient?.dynamicData) {
                              for (const [k, v] of Object.entries(reply.recipient.dynamicData)) {
                                dynamicData[k] = String(v);
                              }
                            }
                            setResponseSubject(renderTemplate(tpl.subject, dynamicData));
                            setResponseText(renderTemplate(tpl.body, dynamicData));
            ...
            <div className="flex justify-between items-center mt-4">
              ...
              <div className="flex items-center gap-3">
                {showSendAgainConfirm ? (
                  <div className="flex items-center gap-2 animate-fade-in bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 rounded">
